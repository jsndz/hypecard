...
const checkProStatus = async (userId) => {
  try {
    ...
    const proEntitlement =
      subscriber.entitlements.pro || subscriber.entitlements.premium;

    return (
      proEntitlement && proEntitlement.expires_date > new Date().toISOString()
    );
  } catch (error) {
    console.error(
      "RevenueCat Pro Status Check Error:",
...
const processWebhookEvent = (event) => {
  try {
    ...
    const isProSubscription =
      productId && (productId.includes("pro") || productId.includes("premium"));
